...
handle_call({register, R}, _From, State) ->
  gen_server:call(State#state.network, {registered, State#state.self}),
...
handle_cast(Msg, State) ->
  ct:pal("dummy_node unhandled cast message: ~p~n", [Msg]),
  {noreply, State}.

handle_info({remote, Msg}, State) ->
  case State#state.respond_to of
    RespondTo when is_pid(RespondTo) ->
      RespondTo ! Msg;
    Other ->
      ct:pal("~p dummy_node cannot send remote message1:~nMessage = ~p~nRespondTo=~p~n", [State#state.self, Msg, Other])
  end,
  {noreply, State};
handle_info(Msg, State) ->
  ct:pal("dummy_node unhandled message: ~p~n", [Msg]),
  {noreply, State}.
