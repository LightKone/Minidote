...
-export([start_link/1, finish/1, deliver_all/1, deliver/4, crash/2, wait_until_registered/2]).
...
-record(state, {
  ...
  registered = maps:new() :: maps:map(Name :: atom(), yes | [From :: pid()])
}).
...
% Wait until a node is registered:
wait_until_registered(LL, Node) ->
  gen_server:call(LL, {wait_until_registered, Node}).
...
handle_call({wait_until_registered, Node}, From, State) ->
  case maps:get(Node, State#state.registered, []) of
    yes ->
      {reply, registered, State};
    Waiting ->
      {noreply, State#state{registered = maps:put(Node, [From | Waiting], State#state.registered)}}
  end;
handle_call({registered, Node}, From, State) ->
  case maps:get(Node, State#state.registered, []) of
    Waiting when is_list(Waiting) ->
      [gen_server:reply(F, registered) || F <- [From | Waiting]];
    _ ->
      ok
  end,
  {noreply, State#state{registered = maps:put(Node, yes, State#state.registered)}};
